      ...
      <div className="relative mx-auto max-w-7xl px-6 lg:px-8">
        ...
        <div className="space-y-8 sm:space-y-12">
          {cards.map((card, index) => {
            ...
            return (
              <ScrollAnimation key={card.title + index} animation={animation}>
                <div
                  className={cn(
                    'relative overflow-hidden rounded-2xl p-6 transition-all duration-300 sm:p-8 lg:p-12',
                    'hover:shadow-xl',
                    cardColors.card,
                  )}
                >
                  ...
                  {card.img && (
                    <div className="absolute right-6 top-6 opacity-10 sm:right-8 sm:top-8">
                      <Image
                        ...
                        width={192}
                        height={128}
                        className="h-24 w-36 rounded-lg object-cover sm:h-32 sm:w-48"
                      />
                    </div>
                  )}
                  ...
                  <div className="relative z-10 grid gap-6 sm:gap-8 lg:grid-cols-2">
                    ...
                    <div>
                      ...
                      {card.icon && (
                        <div
                          className={cn(
                            'mb-4 flex h-16 w-16 items-center justify-center rounded-full transition-all duration-300 sm:mb-6 sm:h-20 sm:w-20',
                      ...
                      {/* Titre */}
                      <h3 className="mb-3 text-2xl font-bold text-foreground sm:mb-4 sm:text-3xl">
                        {card.title}
                      </h3>
                    ...
                    <div className="space-y-3 sm:space-y-4">
                      {card.features.map((feature, featureIndex) => {
                        ...
                        const featureText = isObject
                          ? (feature as FeatureItem).text
                          : feature
                        const featureIcon = isObject
                          ? (feature as FeatureItem).icon
                          : 'CheckCircle'
