...
export default function BaseVariant({
        ...
        <div className="relative">
          ...
          {[
            ...
            <Input
              ...
              onBlur={() =>
                config?.validateOnBlur && validateField(field.name, fieldValue)
              }
  ...
  const renderSection = (section: FormSection, sectionIndex: number) => {
    const sectionColors = colorClasses(
      section.colorVariant ?? colorVariant ?? 'neutral',
    )
    const gridCols =
        ...
        <div className={cn('space-y-6', section.className)}>
          ...
          <div className={cn('grid gap-4 sm:gap-6', gridCols)}>
            {section.fields.map((field, index) =>
              renderField(field, sectionIndex * 10 + index),
            )}
          </div>
  ...
  return (
    ...
    >
      ...
      {/* Formulaire */}
      <form onSubmit={handleSubmit} className={cn('space-y-8', formClassName)}>
        {/* Rendu avec sections */}
        {sections &&
        ...
        {buttons && buttons.length > 0 && (
          <ScrollAnimation animation="slideInUp" delay={0.2}>
            <div
              className={cn(
                'flex flex-col gap-3 border-t border-border pt-6 sm:flex-row',
                buttonAlignClasses[buttonsAlign],
              )}
            >
              {buttons.map((button, index) => {
                const isSubmitButton =
                  button.type === 'submit' || (!button.type && index === 0)
                ...
                return (
                  <Button
                    ...
                    disabled={
                      button.disabled ||
                      (isSubmitButton && (isSubmitting || config?.isDisabled))
                    }
                    className={cn(
