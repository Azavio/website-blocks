        ...
        <div className="mx-auto max-w-6xl">
            ...
            <ScrollAnimation animation="slideInUp" delay={0.1}>
              <div className={cn(
                ...
                {headerRow && (
                  <div className={cn(
                    ...
                    {headerRow.cells.map((cell, cellIndex) => {
                      const cellColors = colorClasses(cell.colorVariant ?? colorVariant ?? "neutral")
                            ...
                            {cell.icon && (
                              <Icon 
                                ...
                                className={cn(
                                  ...
                                  cell.colorVariant ? cellColors.text : "text-muted-foreground"
                                )}
                              />
                            )}
                            <span className={cn(
                              "transition-colors",
                              cell.colorVariant ? cellColors.textContentGradient : "text-foreground"
                            )}>
                              {cell.content}
                            </span>
                            {cell.badge && (
                              <Badge className={cn(
                                "text-xs",
                                cell.colorVariant ? cellColors.badge : "bg-muted text-muted-foreground border-muted"
                              )}>
                                {cell.badge}
                              </Badge>
                            )}
                ...
                {bodyRows.map((row, rowIndex) => (
                  <ScrollAnimation key={rowIndex} animation="slideInUp" delay={0.15 + rowIndex * 0.05}>
                    <div className={cn(
                      ...
                      {row.cells.map((cell, cellIndex) => {
                        const cellColors = colorClasses(cell.colorVariant ?? "neutral")
                        
                        return (
                          <div
                            key={cellIndex}
                            className={cn(
                              "p-4 transition-colors font-medium",
          ...
          <div className="md:hidden space-y-6">
            {bodyRows.map((row, rowIndex) => {
              ...
              const beforeColors = colorClasses(row.cells[1]?.colorVariant ?? "destructive")
              const afterColors = colorClasses(row.cells[2]?.colorVariant ?? "success")
              const improvementColors = colorClasses(row.cells[3]?.colorVariant ?? colorVariant ?? "neutral")
